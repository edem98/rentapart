...
const PropertyHome = ({ user, signIn, navigation }) => {

	const [next, setNext] = useState(null);
	const [url, setUrl] = useState(`${API_CONFIG.server_url}/api/property/list`);
	...
	if (data && data.length === 0) {
		return (
			<NotFound 
				...
				updateData={() => setUrl(`${API_CONFIG.server_url}/api/property/list`)}
			/>
		);
	};
