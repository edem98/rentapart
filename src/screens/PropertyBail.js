...
const PropertyVente = ({ user, signIn, navigation }) => {
	...
	const [url, setUrl] = useState(`${API_CONFIG.server_url}/api/property/list?status=A%20Bailler`);
	...
	if (data && data.length === 0) {
		return (
			<NotFound 
				...
				updateData={() => setUrl(`${API_CONFIG.server_url}/api/property/list?status=A%20Bailler`)}
			/>
		);
	};
